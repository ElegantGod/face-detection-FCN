...
def convert_full_conv():
 # Load the original network and extract the fully connected layers' parameters.
    net = caffe.Net('models/face/deploy.prototxt',
                    'models/face/face.caffemodel',
                    caffe.TEST)
    params = ['fc5', 'fc6']
    fc_params = {pr: (net.params[pr][0].data, net.params[pr][1].data) for pr in params}
    # Load the fully convolutional network to transplant the parameters.
    net_full_conv = caffe.Net('models/face/deploy_fcn.prototxt',
                           'models/face/face.caffemodel',
                              caffe.TEST)
    ...
    net_full_conv.save('models/face/face_fcn.caffemodel')

def face_detection(imgList, minFace=12, maxFace=120):
    net_full_conv = caffe.Net('models/face/deploy_fcn.prototxt','models/face/face_fcn.caffemodel',caffe.TEST)
    img_count = 0
    for imgFile in open(imgList).readlines():
        scales = []
        factor = 0.793700526
        img = Image.open('data/faceImages/'+imgFile.strip())
        min1 = img.size[0] if img.size[0]<img.size[1] else img.size[1]
        ...
        factor_count = 0
        print max2, 'max2'
        while(min1 >= 48 and min1 >= max2):
            scales.append(m1*pow(factor,  factor_count))
            min1 = min1 * factor
            print min1
            factor_count += 1
        total_boxes = []
...
if __name__ == "__main__":
    convert_full_conv()
    face_detection("data/face.txt", 24,120)